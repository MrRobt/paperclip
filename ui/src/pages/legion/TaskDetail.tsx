  ...
  return (
    <aside className="fixed bottom-0 right-0 top-0 z-40 w-[440px] overflow-auto border-l border-border bg-background p-5 shadow-xl">
      ...
      {activeTab === "detail" ? <div className="mt-4 space-y-4 text-sm">
        <section><h3 className="font-medium">基本信息</h3><MarkdownView source={task.description || "无描述"} className="mt-1 text-muted-foreground" /><p className="mt-2 text-xs text-muted-foreground">创建：{new Date(task.createdAt).toLocaleString()} · 更新：{new Date(task.updatedAt).toLocaleString()}</p></section>
        ...
        <section><h3 className="font-medium">验收标准</h3><MarkdownView source={task.verificationCriteria || "未配置"} className="mt-1 text-muted-foreground" /></section>
      </div> : null}
